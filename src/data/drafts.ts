/**
 * Draft posts/products, written but not yet seeded into the database.
 * Proofread at /preview (dev only) before running scripts/seed.mjs, which
 * reads from this same file so what you approve here is what gets pushed.
 *
 * Once a post/product is confirmed live in the DB, remove it from these
 * arrays so seed.mjs doesn't recreate a duplicate on the next run.
 */
...
export const draftPosts = [
  {
    ...
    body: `
      ...
      <ul>
        <li><strong>Phones:</strong> Most modern phones cap fast charging between 20W and 45W. Anything beyond that is wasted on a phone alone.</li>
        <li><strong>Tablets:</strong> Typically 30–45W for reasonable charge times.</li>
        <li><strong>Laptops:</strong> Ultrabooks generally need 65W; larger laptops (16" MacBook Pro class, gaming laptops) often want 96–140W or more.</li>
        <li><strong>Charging multiple devices at once:</strong> Multi-port chargers split their total wattage across active ports, usually unevenly (the primary port gets priority). A "100W" charger feeding a laptop and a phone simultaneously might allocate 65W and 35W rather than 50/50 — check the port-by-port breakdown, not just the headline number.</li>
      </ul>
